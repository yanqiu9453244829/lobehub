...
const { mockFindByTenant, mockGetMessengerWechatConfig, mockRedisSet } = vi.hoisted(() => ({
  mockFindByTenant: vi.fn(),
...
vi.mock('@/database/models/messengerInstallation', () => ({
  MessengerInstallationModel: { findByTenant: mockFindByTenant },
}));
...
beforeEach(() => {
  ...
  mockFindByTenant.mockResolvedValue(installRow);
...
describe('WechatInstallationStore', () => {
  ...
  it('fails closed for an unknown sender without persisting its context token', async () => {
    mockFindByTenant.mockResolvedValueOnce(null);
    const store = new WechatInstallationStore();
